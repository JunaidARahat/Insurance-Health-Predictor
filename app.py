...
import sys
import pip
from insurance.utils.util import read_yaml_file, write_yaml_file
...
from insurance.constants import CONFIG_DIR, get_current_time_stamp
from insurance.pipline.pipline import Pipeline
from insurance.entity.insurance_predictor import predictor, InsuranceData
...
INSURANCE_VALUE_KEY = "healthrisk_value"
...
def predict():
    ...
    if request.method == 'POST':
        ...
        insurance_predictor = predictor(model_dir=MODEL_DIR)
        insurance_value = insurance_predictor.predict(X=insurance_df)[0]

        context = {
            INSURANCE_DATA_KEY: insurance_data.get_insurance_data_as_dict(),
            INSURANCE_VALUE_KEY: float(insurance_value).__round__(2),
        }
